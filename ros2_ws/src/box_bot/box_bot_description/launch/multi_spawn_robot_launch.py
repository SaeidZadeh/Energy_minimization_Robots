...
def gen_robot_list(number_of_robots):
    color_names=["purple", "red", "blue", "green", "yellow", "orange", "brown", "pink", "gold"]
    ...
    for i in range(number_of_robots):
        ...
        y_pos = float(i)-6.5
        robots.append({'name': robot_name, 'x_pose': -7.5, 'y_pose': y_pos, 'z_pose': 0.01, 'color_name': color_names[i], 'color': col_list[i], 'mesh_file':mesh_files[i]})
...
def generate_launch_description():
    ...
    urdf = os.path.join(get_package_share_directory('box_bot_description'), 'robot/model/', 'model1.sdf')
    ...
    for robot in robots:
        i = i+1
        sdf_file = f'model{i}.sdf'
        urdf = os.path.join(get_package_share_directory('box_bot_description'), 'robot/model/', sdf_file)
        pkg_box_bot_description = get_package_share_directory('box_bot_description')
        assert os.path.exists(urdf), "Thebox_bot.urdf doesnt exist in "+str(urdf)
